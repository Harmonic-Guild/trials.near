const StyledButton = styled.button`
  ...
  border: ${(props) =>
    props.variant === "outline" ? "1px solid var(--stroke-color, rgba(255, 255, 255, 0.20))" : ""};
...
function Button({
  ...
  if (href && noLink) {
    return (
      <a href={href} className={linkClassName} style={{ textDecoration: "none" }} target={target}>
        <StyledButton
          id={id}
  ...
  if (href) {
    return (
      <Link to={href} className={linkClassName} style={{ textDecoration: "none" }} target={target}>
        <StyledButton
          id={id}
          key={`ButtonLink-${type ?? "Normal"}-${variant ?? "Default"}-${id}`}
          className={className}
